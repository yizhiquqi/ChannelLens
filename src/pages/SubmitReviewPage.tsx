...
const SUBMITTER_TYPES = [
  { value: 'brand', label: '品牌方/商家' },
  { value: 'creator', label: '达人/主播' },
  { value: 'mcn', label: 'MCN/机构' },
  { value: 'service', label: '服务商/代运营' },
];
const REVIEWER_ROLES = ['创始人', '市场负责人', '渠道负责人', '商务负责人', '电商负责人', '主播/达人', 'MCN负责人', '运营负责人', '其他'];
function submitErrorMessage(error: unknown) {
  ...
  if (detail.includes('row-level security')) {
    return '提交权限校验失败，请退出后重新登录，再提交合作反馈。';
  }
  if (detail.includes('evidence-files') || detail.includes('storage')) {
    return '证据文件上传权限失败，请确认已登录后重新上传材料。';
  }
...
export default function SubmitReviewPage({ onNavigate, user }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.reviewerRole || !form.reviewText || (!form.partnerId && !form.partnerNameFree)) {
      setMessage('请先补齐提交方角色、合作方和合作反馈总结。');
      return;
    }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="max-w-lg w-full bg-white border border-gray-200 rounded-2xl p-10 text-center">
          ...
          <p className="text-gray-500 text-sm leading-relaxed mb-2">
            你的合作反馈和证据材料已提交到后台，平台会进行真实性和证据核验。
          </p>
          <p className="text-gray-400 text-xs mb-6">
            未审核内容不会公开展示；通过审核后，平台会按可见性设置进行匿名脱敏展示。
          </p>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          ...
          <div className="border-b border-gray-100 px-6 py-3 bg-amber-50 flex items-start gap-2">
            ...
            <p className="text-xs text-amber-700">
              合同、付款记录、聊天截图等证据仅用于后台核验，不会公开展示。提交后会进入后台审核，审核通过且设为公开后才会脱敏展示。
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-10">
            {/* 01 提交方信息 */}
            <section>
              <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wide mb-5 pb-2 border-b border-gray-100">01 提交方信息</h2>
              <div className="grid sm:grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">提交方身份 <span className="text-red-400">*</span></label>
                  <select value={form.submitterType} onChange={(e) => f({ submitterType: e.target.value })} className={inputClass} required>
                    {SUBMITTER_TYPES.map((type) => <option key={type.value} value={type.value}>{type.label}</option>)}
                  </select>
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">提交方名称 <span className="text-red-400">*</span></label>
                  <input type="text" placeholder="品牌、达人、MCN或公司名称" value={form.brandName} onChange={(e) => f({ brandName: e.target.value })} className={inputClass} required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">所属类目/领域</label>
                  <input type="text" placeholder="如：美妆、母婴、食品、达人经纪、代运营" value={form.brandCategory} onChange={(e) => f({ brandCategory: e.target.value })} className={inputClass} />
                </div>
